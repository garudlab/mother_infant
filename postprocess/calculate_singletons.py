from utils import sample_utils, config, parse_midas_data, diversity_utils, core_gene_utils
...
singleton_directory = '%s/singleton_rates/%s/' % (config.data_directory, cohort)
...
for species_name in good_species_list:
		...
		while final_line_number >= 0:
				...
				dummy_samples, allele_counts_map, passed_sites_map, final_line_number = parse_midas_data.parse_snps(species_name, debug=debug, allowed_samples=snp_samples, chunk_size=chunk_size,initial_line_number=final_line_number, allowed_genes=non_shared_genes, prev_cohort=cohort)
