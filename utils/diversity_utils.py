...
'''
# ===========================================================================
# Given allele counts for each "passed" site in each sample,
# get the locations of just SNP opportunities
# ===========================================================================

def calculate_opportunity_locations(allele_counts_map, alt_passed_sites_map, allowed_variant_types=set(['1D','2D','3D','4D']), allowed_genes=set([]), lower_threshold=config.consensus_lower_threshold, 
upper_threshold=config.consensus_upper_threshold, min_change=config.fixation_min_change):
		
		# Which genes to consider
		total_genes = set(passed_sites_map.keys())
		allowed_genes = total_genes if len(allowed_genes) == 0 else (allowed_genes & total_genes)
		
		# Result: gene -> variant_type -> location tuple (contig, position) -> NxN matrix of whether a sample pair has opportunity at that site
		opportunity_location_map = {gene: {vt: {} for vt in allowed_variant_types} for gene in allowed_genes}
		
		# Recall: allele_counts_map is
		# gene -> variant_type -> ['alleles', 'locations']
		# 'alleles': list, across sites, of array, across samples, of (A,D) tuples
		# 'locations': list, across sites, of location tuples (contig, position)
		
		# Recall: passed_sites_map is
		# gene -> variant_type -> ['locations', 'sites']
		# 'locations': list, across sites, of location tuples (contig, position)
		# 'sites': sample x sample matrix showing number of sites that "pass" between both samples
		
		arbdict = passed_sites_map.values()[0]
		arbmatrix = arbdict['1D']['sites']
		
		for gene_name in allowed_genes:
			for variant_type in passed_sites_map[gene_name]:
				...
				# Set up list of location tuples lists
				opportunity_location_map[gene_name][variant_type] = [[] for _ in range(arbmatrix.shape[0])]
				
				# matrices: S x N where S is # sites, N is # samples
				depths = allele_counts.sum(axis=2)
				freqs = allele_counts[:,:,0]*1.0/(depths+(depths==0))
				...
				# sites where you could have had a reversion
				reversion_opportunities = derived_sites[:,:,None]*confident_sites
				mutation_opportunities = (passed_sites - missing_data_sites.sum - reversion_opportunities)
				
				ac_locations = allele_counts_map[gene_name][variant_type]
				ps_locations = passed_sites_map[
				
				for rev_opp_matrix, location in zip(reversion_opportunities, ac_locations):
					opportunity_location_map[gene_name][variant_type][location] = numpy.zeros_like(arbmatrix)
					for 
				
				for mut_opp_matrix, location in zip(mutation_opportunities, 
...
upper_threshold=config.consensus_upper_threshold, min_change=config.fixation_min_change):
		...
		mut_fixation_matrix = numpy.zeros_like(arbmatrix).astype('float64')
		rev_fixation_matrix = numpy.zeros_like(mut_fixation_matrix)
		mut_opportunity_matrix = numpy.zeros_like(mut_fixation_matrix)
		rev_opportunity_matrix = numpy.zeros_like(mut_fixation_matrix)
...
#
def calculate_snp_differences_between(i,j,allele_counts_map, passed_sites_map, avg_depth_i, avg_depth_j, min_coverage=config.min_median_coverage, allowed_variant_types=set([]), allowed_genes=set([]), lower_threshold=config.temporal_lower_threshold, 
upper_threshold=config.temporal_upper_threshold, log10_depth_ratio_threshold=config.fixation_log10_depth_ratio_threshold):
		...
		for gene_name in allowed_genes:
				...
				for variant_type in allele_counts_map[gene_name].keys():
						...
						passed_depths = (depths>min_coverage)[:,0]*(depths>min_coverage)[:,1]*(log10_depth_ratios<log10_depth_ratio_threshold)
...
def calculate_highcoverage_samples(species_name, min_coverage=config.min_median_coverage):
		...
		sample_coverage_histograms, samples = parse_midas_data.parse_coverage_distribution(species_name, prev_cohort='any') # note prev cohort 'any'
		...
		# Only include samples that are in 'all samples' (HACKY: TODO?)
		all_samples = sample_utils.get_sample_names('all')
		final_desired_samples = []
		for sample in desired_samples:
			if sample in all_samples:
				final_desired_samples.append(sample)
		
		return final_desired_samples
		


def calculate_haploid_samples(species_name, min_coverage=config.min_median_coverage, threshold_pi=config.threshold_pi, threshold_within_between_fraction=config.threshold_within_between_fraction, prev_cohort='all', debug=False):
		...
		import sfs_utils
		samples, sfs_map = parse_midas_data.parse_within_sample_sfs(species_name,			allowed_variant_types=set(['4D']), prev_cohort=prev_cohort) 
...
# Considers mothers and infants the same subject
def calculate_temporal_samples(species_name, min_coverage=config.min_median_coverage):
		...
		same_sample_idxs, same_subject_idxs, diff_subject_idxs = sample_utils.calculate_ordered_subject_pairs(sample_order_map, highcoverage_samples, within_host_type = 'nonconsecutive')
		...
		for sample_pair_idx in xrange(0,len(diff_subject_idxs[0])):
				i = diff_subject_idxs[0][sample_pair_idx]
				j = diff_subject_idxs[1][sample_pair_idx]
				sample_i = highcoverage_samples[i]
				sample_j = highcoverage_samples[j]
				if sample_utils.is_mi_pair(sample_i, sample_j, mother_samples, infant_samples) and sample_utils.is_same_mi_subject(sample_i, sample_j, sample_subject_map):
					temporal_samples.add(sample_i)
					temporal_samples.add(sample_j)
...
def calculate_fixation_error_rate(sfs_map, sample_i, sample_j,dfs=[0.6, 0.3], frequency_bins = numpy.linspace(0,1,21)):
		...
		for D1,pD1 in zip(D1s,pD1s):
				for D2,pD2 in zip(D2s,pD2s):
						for f,pf in zip(fs,pfs): # True f
								for df in dfs: # df = 0.6 			# (1-0.6)/2 = 0.2
										# P(true frequency is f | alt allele frequency f* <= 0.2) at timepoint 1
										# P(true frequency is 1-f | alt allele frequency f* >= 0.8) at timepoint 2
										# pD1: proportion of sites that have this depth
										# pD2: proportion of sites that have this depth
										# freq * depth 1 * depth 2
