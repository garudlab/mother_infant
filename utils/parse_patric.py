...
def load_kegg_annotations(genome_ids):
		...
		for genome_id in genome_ids:
				file = open("%skegg/%s.kegg.txt" % (config.patric_directory, genome_id),"r")
	# file= bz2.BZ2File("%skegg/%s.kegg.txt.bz2" % (config.patric_directory, genome_id),"r")
...
def load_spgenes_annotations(gene_names):
		...
		for gene_name in gene_names: 
				...
				if genome_id not in genomes_visited:
						...
						file= gzip.open("%spatric_spgene/%s.PATRIC.spgene.tab.gz" % (config.patric_directory, genome_id),"r")
						file.readline() #header	 
						for line in file:
								if line.strip() != "":
										items = line.split("\t")
...
def load_patric_gene_descriptions(genome_ids,	 allowed_genes=[]):
		...
		for genome_id in genome_ids:
				# file = open('%s/genomes/%s/%s.PATRIC.features.tab' % (config.patric_directory, genome_id, genome_id), 'r')
				try:
					file=gzip.open('%s/features/%s.PATRIC.features.tab.gz' % (config.patric_directory, genome_id), 'r')
				except:
					print("PATRIC file for " + genome_id + " not found!")
					continue
				file.readline() #header	 
				for line in file:
...
#################################################################
#
# Load gene name from patric for a specific gene
#
#################################################################

def load_patric_gene_name(gene_id):
		...
		for line in file:
				...
				if items[0] !='':
						if items[5] !='' and len(items)>14: # sometimes entries are blank
								if gene_id == items[5].split('|')[1]:
										return items[14] # what the gene does
