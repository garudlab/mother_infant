...
if species is None:
		...
		outFile=open('%sgene_changes_shuffle_%s.txt' % (config.analysis_directory, species) , 'w')
...
cohort = mother_samples + infant_samples
...
for species_name in good_species_list: 
		...
		#
		same_subject_snp_idxs = su.calculate_mi_ordered_subject_pairs(sample_order_map, sample_list=desired_samples, within_host_type='consecutive', one_per_mi_pair=True, infant_timepoint_pref='first')
		...
		for sample_pair_idx in xrange(0,len(same_subject_snp_idxs[0])):
				...
				#
				# First SNP changes
				snp_opportunities, perr, mutations, reversions = calculate_temporal_changes.calculate_mutations_reversions_from_temporal_change_map(temporal_change_map, sample_i, sample_j, lower_threshold, upper_threshold)
				#
				...
				for change_type in within_host_classes:
						for trial in range(0,num_trials):
								try: # TODO: Figure out why this doesn't work
		...
		for change_type in within_host_classes:
				...
				for gene_id in within_host_changes_gene_ids[change_type]:
						# Skip over genes with no kegg annotation
						if gene_id not in kegg_ids.keys() or gene_id not in gene_descriptions:
...
if species is None:
		pickle.dump(all_data, open("%s/pickles/gene_changes_mi/all_species_gene_changes.p" % config.data_directory, "wb"))		
		pickle.dump(bad_genes, open("%s/pickles/gene_changes_mi/bad_genes.pkl" % config.data_directory, "wb"))		
		pickle.dump(genes_of_interest, open("%s/pickles/gene_changes_mi/gene_changes_of_interest_%s.pkl" % (config.data_directory, sweep_type), "wb"))
else:
		pickle.dump(all_data, open("%s/pickles/gene_changes_mi/%s_gene_changes.p" % (config.data_directory, species), "wb"))
		pickle.dump(bad_genes, open("%s/pickles/gene_changes_mi/%s_bad_genes.pkl" % (config.data_directory, species), "wb"))		
		pickle.dump(genes_of_interest, open("%s/pickles/gene_changes_mi/%s_gene_changes_of_interest_%s.pkl" % (config.data_directory, species, sweep_type), "wb"))
