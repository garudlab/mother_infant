# Co-opted from plot_figure_3.py
...
debug_species = ["Bacteroides_vulgatus_57955", "Bacteroides_uniformis_57318"]
good_species_list = debug_species if debug else parse_midas_data.parse_good_species_list()
...
mi_samples = sample_utils.get_sample_names('all','all')
...
for species_name in good_species_list:
		...
		# Take specific mother and baby timepoints if possible
		snp_samples = snp_samples[sample_utils.calculate_unique_samples_consistent(subject_sample_map, sample_list=snp_samples, mother_tp=(1,1,2), infant_tp=(2,5,5))]
		
		sys.stderr.write("Taking only mother and infant samples...\n")
		snp_samples = [s for s in snp_samples if s in mi_samples]
		...
		for i in xrange(0, syn_difference_matrix.shape[0]):
				for j in xrange(i+1, syn_difference_matrix.shape[0]):						
						if syn_opportunity_matrix[i,j]>0 and non_opportunity_matrix[i,j]>0 and sample_utils.is_mi_pair(snp_samples[i],snp_samples[j]):
...
outputfd = open('dn_ds_mother-mother.txt', 'w')
outputfd.write(str(species_names) + '\n')
outputfd.write(str(len(species_names)))
...
for species_idx in xrange(0,len(species_names)):
		...
		if False and species_name.startswith('Bacteroides'):
				...
				# divergence_axis.loglog(pS2s[good_same_subj_idxs], pNpSs[good_same_subj_idxs], '.', color='#dfff00', markersize=2,alpha=0.5,markeredgewidth=0,zorder=0,rasterized=True)
...
# fig.savefig('%s/dNdS_mi_pairs_only.pdf' % (parse_midas_data.analysis_directory),bbox_inches='tight',dpi=600)
# fig2.savefig('%s/singleton_dNdS_mi_pairs_only.pdf' % (parse_midas_data.analysis_directory),bbox_inches='tight',dpi=600)
sys.stderr.write("Done!\n")
