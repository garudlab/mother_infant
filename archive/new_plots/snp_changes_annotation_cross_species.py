# Within-host snp gene change annotation
...
pdir = "%s/pickles" % ddir
prev_cohort = 'prev_hmp'
all_data={}
for species_name in good_species_list:
		print species_name
		if os.path.isfile("%s/gene_changes_anno_old_%s_%s.pkl" % (pdir, species_name, prev_cohort)):
				all_data_species = pickle.load( open( "%s/gene_changes_anno_old_%s_%s.pkl" % (pdir, species_name, prev_cohort), "rb" ))
						...
						if gene_type in all_data[species_name][cohort]:
							for gene in all_data[species_name][cohort][gene_type].keys():
									if gene not in all_data['all_species'][cohort][gene_type].keys():
											all_data['all_species'][cohort][gene_type][gene] = {'snps':[0,0,0,0]} #[obs, exp_between, exp_present, exp_pangeome]
						...
						for null_type in ['between_host_genes', 'present_genes', 'pangenome_genes']:
								for gene in all_data[species_name][cohort]['null'][null_type].keys():
										if gene not in	all_data['all_species'][cohort]['null'][null_type].keys():
												all_data['all_species'][cohort]['null'][null_type][gene]={'snps':[0]*num_trials}
										for change_type in ['snps']:
...
for cohort in cohorts:
	...
	outFile_gene_change=open('%s/snp_changes_across_species_%s.txt' %	 (config.analysis_directory, cohort) ,'w')
...
for cohort in cohorts:
	...
	for gene in all_data['all_species'][cohort]['gene_changes']:
		if all_data['all_species'][cohort]['gene_changes'][gene]['snps'][0] > 0: 
			keyword_found=False
	...
	outFile_keywords=open('%s/snp_changes_across_species_keywords_%s.txt' %	 (config.analysis_directory, cohort),'w')
