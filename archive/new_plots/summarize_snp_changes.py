...
pickle_fname = '%s/pickles/dNdS_distribution_partial.pkl' % (config.data_directory)
dNdS_distribution = pickle.load(open(pickle_fname, 'rb'))

pickle_fname = '%s/pickles/mod_dNdS_distribution_full.pkl' % (config.data_directory)
mod_dNdS_distribution = pickle.load(open(pickle_fname, 'rb'))

# All, partial
nonsyn_diff_total = {e: defaultdict(int) for e in event_types}
nonsyn_opp_total = {e: defaultdict(int) for e in event_types}
syn_diff_total = {e: defaultdict(int) for e in event_types}
syn_opp_total = {e: defaultdict(int) for e in event_types}

ndiff_partial = defaultdict(int)
nopp_partial = defaultdict(int)
sdiff_partial = defaultdict(int)
sopp_partial = defaultdict(int)

for event_type in dNdS_distribution:
	for species in dNdS_distribution[event_type]:
		for cohort in dNdS_distribution[event_type][species]:
			for tp_pair in dNdS_distribution[event_type][species][cohort]:
				cat = tp_to_category(tp_pair)
				if cat == 'IM':
					cat = 'MI'
				ndiff, nopp, sdiff, sopp = dNdS_distribution[event_type][species][cohort][tp_pair]
				
				nonsyn_diff_total[event_type][cat] += ndiff
				nonsyn_opp_total[event_type][cat] += nopp
				syn_diff_total[event_type][cat] += sdiff
				syn_opp_total[event_type][cat] += sopp
				
				if event_type == 'modification':
					ndiff_partial[cat] += ndiff
					nopp_partial[cat] += nopp
					sdiff_partial[cat] += sdiff
					sopp_partial[cat] += sopp

for e in ['modification', 'replacement']:
	for tp_type in nonsyn_diff_total[e]:
		dN = (nonsyn_diff_total[e][tp_type]/nonsyn_opp_total[e][tp_type])
		dS = (syn_diff_total[e][tp_type]/syn_opp_total[e][tp_type])
		if dS == 0:
			print("dS zero for %s, %s" % (tp_type, e))
		else:
			print("dN/dS for %s, %s is %.05f" % (tp_type, e, (dN/dS)))

# Modification, full only
nonsyn_diff_total = defaultdict(int)
nonsyn_opp_total = defaultdict(int)
syn_diff_total = defaultdict(int)
syn_opp_total = defaultdict(int)

for species in mod_dNdS_distribution:
	for cohort in mod_dNdS_distribution[species]:
		for tp_pair in mod_dNdS_distribution[species][cohort]:
			cat = tp_to_category(tp_pair)
			if cat == 'IM':
				cat = 'MI'
			ndiff, nopp, sdiff, sopp = mod_dNdS_distribution[species][cohort][tp_pair]
			nonsyn_diff_total[cat] += ndiff
			nonsyn_opp_total[cat] += nopp
			syn_diff_total[cat] += sdiff
			syn_opp_total[cat] += sopp

for tp_type in nonsyn_diff_total:
	dN = (nonsyn_diff_total[tp_type]/nonsyn_opp_total[tp_type])
	dS = (syn_diff_total[tp_type]/syn_opp_total[tp_type])
	print("dN/dS for %s is %.05f" % (tp_type, (dN/dS)))
...
mod_or_replace_not_sure_full = []
mod_or_replace_not_sure_all = []
	...
	for tp_pair in full_subset.keys():
		...
		for snp_change_count in full_subset[tp_pair]:
			if snp_change_count == 0:
				...
				mod_or_replace_not_sure_full.append(snp_change_count)
		# All
		for snp_change_count, full_snp_change_count, partial_snp_change_count, partial_percent in zip(all_subset[tp_pair], full_subset[tp_pair], partial_subset[tp_pair], partial_percent_subset[tp_pair]):
			if snp_change_count == 0:
				...
				mod_or_replace_not_sure_all.append(snp_change_count)
