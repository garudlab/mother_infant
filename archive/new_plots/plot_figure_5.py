import matplotlib	 
matplotlib.use('Agg') 
from utils import sample_utils, config, parse_midas_data, diversity_utils, gene_diversity_utils, temporal_changes_utils, substitution_rates_utils, stats_utils, sfs_utils, snps_utils
import os.path
import pylab
import sys
import numpy
...
output_filename = ('%s/within_host_output_%s.txt' % (config.analysis_directory, prev_cohort))
output_strs = []
...
#
#####
cohorts = ["hmp", "backhed", "shao", "ferretti", "yassour", "olm"]
sample_cohort_map = sample_utils.parse_sample_cohort_map()

modification_difference_thresholds = {cohort: modification_difference_threshold for cohort in cohorts}

replacement_difference_thresholds = {cohort: replacement_difference_threshold for cohort in cohorts}
...
cohort_species_qp_counts = {cohort: {} for cohort in cohorts}
...
for species_name in good_species_list:
		...
		same_subject_idxs = sample_utils.calculate_mi_ordered_same_subject_pairs(sample_order_map, all_samples, within_host_type=within_host_type, one_per_mi_pair=False)
		...
		for sample_pair_idx in xrange(0,len(same_subject_idxs[0])):
				...
				cohort = sample_cohort_map[sample_i]
		...
		cohort_species_qp_counts[cohort][species_name] = qp_counts[cohort]
		...
		snv_freq_map = snps_utils.parse_population_freqs('all', species_name,polarize_by_consensus=True)
		snv_freq_keys = snv_freq_map.keys()
		snv_freq_values = snv_freq_map.values()
		
		from utils import core_gene_utils
		gene_freq_map = core_gene_utils.parse_gene_freqs(species_name, prev_cohort=prev_cohort)
		...
		dummy_samples, snp_mut_difference_matrix, snp_rev_difference_matrix, snp_mut_opportunity_matrix, snp_rev_opportunity_matrix = substitution_rates_utils.calculate_mutrev_matrices_from_substitution_rate_map(substitution_rate_map, 'all', allowed_samples=combined_samples)
		...
		gene_samples, gene_loss_difference_matrix, gene_gain_difference_matrix, gene_loss_opportunity_matrix, gene_gain_opportunity_matrix = substitution_rates_utils.calculate_mutrev_matrices_from_substitution_rate_map(substitution_rate_map, 'genes', allowed_samples=combined_samples)
		...
		for var_type in variant_types:
				...
				# First do HMP
				dummy_samples, difference_matrix, opportunity_matrix =		substitution_rates_utils.calculate_matrices_from_substitution_rate_map(substitution_rate_map, var_type, allowed_samples=combined_samples)
		...
		for cohort in cohorts:
				...
				same_subject_idxs = sample_utils.calculate_mi_ordered_same_subject_pairs(sample_order_map, desired_samples, within_host_type=within_host_type, one_per_mi_pair=False)
				...
				for sample_pair_idx in xrange(0,len(same_subject_idxs[0])):
						...
						good_idxs = sample_utils.calculate_samples_in_different_subjects( sample_subject_map, combined_samples, sample_i)
						good_idxs *= ( (snp_opportunity_matrix[i,:]>0.5) * (gene_opportunity_matrix[i,:]>0.5) )
						
						if good_idxs.sum() < 1:
								continue
								
						L, perr, mutations, reversions = temporal_changes_utils.calculate_mutations_reversions_from_temporal_change_map(temporal_change_map, sample_i, sample_j)
...
pickle.dump(species_snp_change_distribution, open('%s/pickles/species_snp_change_distribution_prev_%s.pkl' % (config.data_directory, prev_cohort), 'wb'))
pickle.dump(species_snp_nerrs, open('%s/pickles/species_snp_nerrs_%s.pkl' % (config.data_directory, prev_cohort), 'wb'))
pickle.dump(species_gene_change_distribution, open('%s/pickles/species_gene_change_distribution_prev_%s.pkl' % (config.data_directory, prev_cohort), 'wb'))
pickle.dump(species_gene_nerrs, open('%s/pickles/species_gene_nerrs_%s.pkl' % (config.data_directory, prev_cohort), 'wb'))
pickle.dump(pooled_snp_change_distribution, open('%s/pickles/pooled_snp_change_distribution_prev_%s.pkl' % (config.data_directory, prev_cohort), 'wb'))
pickle.dump(pooled_gene_change_distribution, open('%s/pickles/pooled_gene_change_distribution_prev_%s.pkl' % (config.data_directory, prev_cohort), 'wb'))
pickle.dump(pooled_snp_length_distribution, open('%s/pickles/pooled_snp_length_distribution_prev_%s.pkl' % (config.data_directory, prev_cohort), 'wb'))
pickle.dump(pooled_gene_length_distribution, open('%s/pickles/pooled_gene_length_distribution_prev_%s.pkl' % (config.data_directory, prev_cohort), 'wb'))
pickle.dump(pooled_between_snp_change_distribution, open('%s/pickles/pooled_between_snp_change_distribution_prev_%s.pkl' % (config.data_directory, prev_cohort), 'wb'))
pickle.dump(pooled_between_gene_change_distribution, open('%s/pickles/pooled_between_gene_change_distribution_prev_%s.pkl' % (config.data_directory, prev_cohort), 'wb'))
pickle.dump(pooled_min_between_snp_change_distribution, open('%s/pickles/pooled_min_between_snp_change_distribution_prev_%s.pkl' % (config.data_directory, prev_cohort), 'wb'))
pickle.dump(pooled_min_between_gene_change_distribution, open('%s/pickles/pooled_min_between_gene_change_distribution_prev_%s.pkl' % (config.data_directory, prev_cohort), 'wb'))
pickle.dump(replacement_map, open('%s/pickles/replacement_map_prev_%s.pkl' % (config.data_directory, prev_cohort), 'wb'))
pickle.dump(total_freq_snps, open('%s/pickles/total_freq_snps_prev_%s.pkl' % (config.data_directory, prev_cohort), 'wb'))
pickle.dump(total_null_freq_snps, open('%s/pickles/total_null_freq_snps_prev_%s.pkl' % (config.data_directory, prev_cohort), 'wb'))
...
for cohort in cohorts:
		...
		if cohort=='hmp':
				...
				fig2.savefig('%s/figure_6.1.pdf' % (config.analysis_directory),bbox_inches='tight',transparent=True)
				...
				# Save intermediate version (for Keynote animations)
				fig2.savefig('%s/figure_6.2.pdf' % (config.analysis_directory),bbox_inches='tight',transparent=True)
				...
				# Save intermediate version (for Keynote animations)
				fig2.savefig('%s/figure_6.3.pdf' % (config.analysis_directory),bbox_inches='tight',transparent=True)
...
fig2.savefig('%s/%s/figure_5.pdf' % (config.analysis_directory, prev_cohort),bbox_inches='tight')
fig.savefig('%s/%s/supplemental_within_across_species.pdf' % (config.analysis_directory, prev_cohort),bbox_inches='tight')
sys.stderr.write("Done!\n")
# fig3.savefig('%s/supplemental_young_twin_within.pdf' % (config.analysis_directory),bbox_inches='tight',transparent=True)
fig4.savefig('%s/%s/supplemental_within_between_avg.pdf' % (config.analysis_directory, prev_cohort),bbox_inches='tight',transparent=True)
# fig5.savefig('%s/supplemental_twin_modification_frequency.pdf' % (config.analysis_directory),bbox_inches='tight',transparent=True)
#fig6.savefig('%s/supplemental_temporal_qp_sample_size.pdf' % (config.analysis_directory),bbox_inches='tight',transparent=True)
fig8.savefig('%s/%s/supplemental_within_across_species.pdf' % (config.analysis_directory, prev_cohort),bbox_inches='tight',transparent=True)
fig9.savefig('%s/%s/supplemental_within_ks.pdf' % (config.analysis_directory, prev_cohort),bbox_inches='tight',transparent=True)
fig10.savefig('%s/%s/supplemental_sites_retained.pdf' % (config.analysis_directory, prev_cohort),bbox_inches='tight',transparent=True)
